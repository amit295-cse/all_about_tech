...
mail = Mail(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/falskproject'
db = SQLAlchemy(app)
...
def edit(sno):
  if 'user' in session and session['user'] == params['admin-user']:
     if(request.method == 'POST'):
         ...
         imgf= request.form.get('title')
         date = datetime.now()


         if (sno=='0'):
          post = Posts(title= box_title, slug= slug , content= content ,tagline= tline, date= date,img_file= imgf)
          db.session.add(post)
          db.session.commit()
